...
serviceUUID = "productpageservice-{}".format(uuid.uuid4().hex)
FI_TRACE = True
...
def front():
    ...
    if FI_TRACE:
        ...
        data["records"].append(generate_record(uuid=reviews_req_uuid, type=1, message_name="Products Review Request", service=serviceUUID))
        ...
        data["records"].append(generate_record(uuid=reviews_req_uuid, type=2, message_name="Product Review Response", service=serviceUUID))

        # Record Response to client
        message_name = "Product Page Response"
        data["records"].append(generate_record(uuid=id, type=1, message_name=message_name, service=serviceUUID))
    else:
